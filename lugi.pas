...
CONST
    ...
    initfilespec = 'lugi-init.txt';  {**** filespec for opening (init) for initialization}
...
{Returns a random meaning in vnorth..(drange).}
	...
	x: converter;
    BEGIN
    x.cvi := Trunc(Random * (1 + Ord(drange) - Ord(vnorth))) + Ord(vnorth);
    RandDirection := x.cvm
    END;  {RandDirection}
...
{adjacent to (avoid).  Use Distant(nowhere) to get any random room.	 }
    ...
    BEGIN
    REPEAT
	x.cvi := Trunc(Random * Ord(pbalcony)) + 1
...
IF NOT ready THEN BEGIN
    Assign(init,initfilespec); Reset(init);  {**** open the file}
    LoadMessages;
    IF ready THEN
	MakeDictionary
    ELSE
